...
export default async function IhaleDetay({
  ...
  // "Projeyi İndir" butonu yalnızca giriş yapmış kullanıcılara (müteahhitler
  // dahil) görünür. "ihale-belgeleri" bucket'ı public olduğundan Supabase
  // Storage'ın "?download" sorgu parametresi ile dosya, yeni sekmede
  // açılmak yerine doğrudan indirilir.
  const { data: { user } } = await supabase.auth.getUser();
  const projeIndirUrl = projeBelgeUrl
    ? `${projeBelgeUrl}${projeBelgeUrl.includes("?") ? "&" : "?"}download`
    : null;
