...
function OdemeBildirimi() {
  ...
  const durum = searchParams.get("odeme"); // "basarili" | "uyari" | null
  const [goster, setGoster] = useState(() => durum === "basarili" || durum === "uyari");

  useEffect(() => {
    if (!goster) return;
    const t = setTimeout(() => setGoster(false), durum === "uyari" ? 15000 : 6000);
  ...
  if (durum === "uyari") {
    return (
      <div className="bg-amber-50 border border-amber-200 rounded-2xl px-5 py-4 mb-5 flex items-center gap-4">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-amber-800">Ödemeniz alındı, ancak hesabınıza yansıtılırken bir sorun oluştu.</p>
          <p className="text-xs text-amber-700 mt-0.5">Lütfen destek ile iletişime geçin, ödemeniz kayıt altına alındı.</p>
        </div>
        <button
