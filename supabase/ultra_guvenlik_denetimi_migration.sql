-- ULTRA DETAYLI guvenlik denetiminde bulunan KRITIK/YUKSEK/ORTA
-- bulgularin duzeltmesi. Idempotent, tek "Run" ile guvenle calisir.
-- Bu migration'i calistirmadan once dev/staging'de test edilmesi
-- onerilir (buyuk bir migration).
...
CREATE OR REPLACE VIEW public.kullanicilar_ozet
...
-- ekleniyor -- aksi halde yukaridaki trigger bunlari da engellerdi.
...
-- ============================================================
-- 5) YUKSEK (veri tutarliligi): teklif silinince (or. hesap silme ile
-- CASCADE) mevcut_teklif guncellenmiyordu, artik var olmayan bir
-- teklifi gostermeye devam ediyordu.
-- ============================================================
...
SELECT tgname FROM pg_trigger WHERE tgname IN ('trg_kullanici_kisitli_sutun_kontrol', 'on_teklif_degisti');
