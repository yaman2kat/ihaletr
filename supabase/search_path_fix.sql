-- ============================================================
-- IhaleTR - Add "SET search_path = public" to SECURITY DEFINER
-- trigger/RPC functions.
-- Paste this into Supabase Dashboard > SQL Editor and run it.
-- Plain ASCII only (no Turkish characters) to avoid encoding issues.
-- ============================================================
--
-- Why: these functions are invoked by triggers on auth.users, which
-- run as the supabase_auth_admin role. That role's default
-- search_path may not include "public", so unqualified type/function
-- names inside the function body (like "hesap_turu_tipi") can fail
-- to resolve even though the type genuinely exists in public.
-- Adding "SET search_path = public" to the function definition makes
-- Postgres always resolve unqualified names against public first,
-- no matter which role or session calls the function.
--
-- This does NOT change any function logic - only adds the SET
-- clause. Bodies below are the latest versions (including the
-- EXCEPTION WHEN OTHERS guards already added previously).

-- ── 1) handle_new_user() — trigger on auth.users (AFTER INSERT) ──
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'handle_new_user failed (auth.users id=%): %', NEW.id, SQLERRM;
    RETURN NEW;
END;
$$;

-- ── 2) handle_davet_odul_kaydi() — trigger on auth.users (AFTER UPDATE) ──
...
EXCEPTION
  WHEN OTHERS THEN
    RAISE WARNING 'handle_davet_odul_kaydi failed (auth.users id=%): %', NEW.id, SQLERRM;
    RETURN NEW;
END;
$$;

-- ── 3) set_updated_at() — BEFORE UPDATE trigger on several tables ──
-- Not SECURITY DEFINER and does not reference any custom type/table,
-- so it was never actually at risk, but SET search_path = public is
-- added anyway for consistency across all trigger functions.
...
-- ── 4) davet_odulunu_baslat() — called from both triggers above ──
...
-- ============================================================
-- The 4 functions above are the ones requested. The functions below
-- are the SAME class of SECURITY DEFINER function referencing
-- unqualified public-schema types (hesap_turu_tipi, odul_turu) and
-- are called from the same signup/login paths (oauth_kayit_tamamla
-- is invoked directly from src/app/auth/callback/route.ts). Adding
-- search_path here too closes the same gap before it causes another
-- outage. Logic is unchanged from hesap_turu_migration.sql.
-- ============================================================

-- ── 5) gen_davet_kodu() — called from handle_new_user() ──
...
-- ── 6) oauth_kayit_tamamla() — called from /auth/callback ──
...
-- ── 7) davet_odulu_uygula() — RPC called from the client ──
...
BEGIN
  -- Note: the RAISE EXCEPTION strings below are shown to end users in
  -- the app and are kept in their original Turkish wording on purpose
  -- (unchanged from hesap_turu_migration.sql) - only the function
  -- signature (SET search_path) changed here.
...
-- ── VERIFY: confirm search_path is now set on all of them ──────────
select
  p.proname as function_name,
  p.prosecdef as is_security_definer,
  p.proconfig as config_settings   -- should show {search_path=public}
from pg_proc p
join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
  and p.proname in (
    'handle_new_user', 'handle_davet_odul_kaydi', 'set_updated_at',
    'davet_odulunu_baslat', 'gen_davet_kodu', 'oauth_kayit_tamamla',
    'davet_odulu_uygula'
  )
order by p.proname;
